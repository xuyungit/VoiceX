...
### Added
- **Translate and read** *(macOS)* — select text, press **⌥⌘T** (configurable), and VoiceX has the LLM translate it into the target language (English by default) and reads the translation aloud. It has its own hotkey, its own LLM picker (follow the LLM page, a provider, or a custom endpoint) and optional per-language voice overrides. The HUD shows a wand while the model works; the key or **Escape** cancels, and any reading key stops a read in progress. Selections up to 5000 characters are sent. A failed, timed-out or unconfigured LLM is reported on the HUD and never falls back to reading the untranslated text. The source and translation can be kept on the History page (on by default) and optionally copied to the clipboard.
- **Captions on the HUD** *(macOS)* — with captions on (the default), the HUD switches to a text-only layout and shows the sentence being spoken, for plain and translated reads alike. Text is synthesized sentence by sentence, one cloud request per ~120-character piece; `qwen-audio-3.1-tts-flash` is the exception and keeps a read in as few requests as the service takes, placing captions from the service's own per-character timing. The default system voice (`say`) reports no progress, so it keeps the compact HUD.
- **Read the clipboard when nothing is selected** *(macOS)* — both reading keys fall back to the clipboard's plain text when the selection is empty or cannot be read. The HUD labels such a read "Clipboard". Secure input, missing permissions and a changed foreground app still report their own errors, and content a password manager marks as concealed is never read. On by default; the selection's Copy fallback is now called the compatibility mode to keep the two apart.
- **Tidy before reading** *(macOS, opt-in)* — a plain read can go through the LLM first to strip Markdown, HTML markup and link URLs. If tidying fails, the original text is read.
- **Qwen-Audio 3.1** — `qwen-audio-3.1-tts-flash` is now the default Alibaba Cloud reading model: the same endpoint as 3.0 but billed by audio token, about a sixth of the price for the same text, with its own 41-voice table (3.1 rejects every 3.0 voice id). `qwen-audio-3.1-asr-flash-streaming` and `qwen-audio-3.1-asr-flash` join the Qwen ASR model list with 3.0's hotword, context and VAD options. See `docs/qwen-audio-3.1-research-2026-09-23.md`, including where 3.1 streaming ASR costs more than 3.0 (short utterances sent with a dictionary).
- **Reading style instruction for Alibaba Cloud** *(macOS)* — Qwen-Audio 3.x and CosyVoice v3.5 accept a natural-language delivery instruction. The default asks for a calm, even delivery suited to technical text, which measurably softens the timbre jump where the service starts a new chunk. Models that do not take one (CosyVoice v3 system voices, Qwen3-TTS) hide the field and send nothing.
- **Reading statistics on the Overview** — a second section counts plain reads, translated reads, characters read, reading time and the LLM calls reads triggered, per device and account-wide. A read counts once the engine has accepted it. With sync on, per-device totals travel on a new sync-server endpoint (`PUT /v1/usage/tts`); an older server answers 404 and history sync is unaffected, so the two can be upgraded in either order.

### Changed
- **Every LLM is asked for its lowest reasoning by default** — correction and translation of short text gain nothing from thinking and lose seconds to it. The LLM page defaults every provider to "Auto lowest", resolved per provider, host and model (`thinking: disabled` on Volcengine Ark, `reasoning_effort` where the vendor honours it, nothing for an unknown custom host), and shows the exact fields that will be sent. A stored Volcengine `minimal` migrates to the new default. OpenAI and custom endpoints can also name a level or send none, and custom endpoints take an **extra request fields** JSON object merged over the request body. The measurements behind each rule are in `docs/llm-reasoning-knobs-2026-09-20.md`.
- **No fixed output cap on LLM requests** — the 4096-token cap on OpenAI, Qwen, Gemini and custom providers let reasoning models spend the whole budget thinking, so long translations came back cut. The cap is gone, and a reply that stopped at the output limit is now an error on every API shape instead of being used as-is.
- **Tables are read as sentences** — the reading and translation prompts now recognise a table copied from a web page (tab-separated rows), open with what the table is about and read each row as one sentence, instead of reading cell by cell. Code blocks get a one-line summary.
- **The Reading settings page is regrouped** by hotkey and engine: plain reading and translate-and-read are two hotkey rows with their own switches, and voice, rate, pitch and volume sit under the engine that owns them, showing only the sliders that engine honours.
- Translate-and-read history rows stay on this device: they are not uploaded by sync and do not count toward dictation usage.

### Fixed
- **Digit keys in hotkeys were recorded as other keys** — digits were numbered by ASCII while special keys used macOS keycodes, so seven digits collided: `Ctrl+Opt+Cmd+1` was shown and stored as `… + Space`, and a bare `7` counted as a modifier-only binding. Digits now have their own codes. Stored bindings on `2`, `4` and `9` are migrated automatically; **a hotkey recorded on `0`, `1`, `3`, `5`, `6`, `7` or `8` has to be recorded again**, because the stored value cannot say which key was pressed.
- **Alibaba Cloud read numbers with pauses** — text with a space on each side of a number (which the LLM stages produce routinely) was spoken by Qwen-Audio 3.x as 「有、八、个」. The space between Chinese text and a token containing a digit is now closed before synthesis; captions still show the text as written.
- **A caption piece could be cut inside a number or a file name** (`10.05`, `1.13%`, `window.rs`), so each half was normalised on its own. ASCII punctuation is a sentence boundary only when no letter or digit follows it.
- Stopping a read before its first audio byte is a clean stop rather than a decode error in the log.

### Developer tooling
- `tools/llm-bench` scores correction by diffing each case's input against its expected text (dictionary, semantic and filler-cleanup sites, settled by code where possible and by the TypeSafe judge otherwise), times latency on a log scale against the app's own correction timeout, keeps every run's report and results on disk, and separates standings by scoring version.
- `docs/tts-regression-testing.md` and `scripts/tts/` describe and drive the reading regression runs, including HUD lifecycle probes, with hotkeys read from the app's settings.
