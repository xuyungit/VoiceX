...
- **朗读与翻译朗读** — 在任意应用里选中文字，一个热键读出来；另一个热键先交给大模型翻译再朗读。HUD 逐句显示字幕，没选中文字时读剪贴板。可选系统语音或多家云端 TTS（目前仅 macOS）。
...
- **历史记录与统计** — 每次听写保留完整元数据（时长、设备、ASR/LLM 模型、原文 vs. 纠正文本），按日期浏览，并支持录音回放、重新转录和重放注入测试；概览页同时统计听写和朗读。
...
除了把语音变成文字，VoiceX 也能把文字读出来。在**任意应用**里选中一段文本：

| 热键（可改） | 作用 |
|---|---|
| **⌥⌘R** 朗读 | 直接读出选中的文字 |
| **⌥⌘T** 翻译朗读 | 先交给大模型翻译成目标语言（默认英文），再读出译文；已经是目标语言的文本只做整理、不翻译 |

再按一次任一朗读热键或按 **Escape** 立即停止；等待翻译时按热键或 Escape 取消。

> **目前仅 macOS。** 从其他应用取词依赖 macOS 辅助功能接口，Windows 版尚未实现，这两个热键在 Windows 上不会被注册，按下时仍然交给前台应用。
...
| | 说明 |
|---|---|
| **取词方式** | 优先走辅助功能接口直接取（8–15 ms）；取不到时降级为模拟 Command + C，并在读完后还原剪贴板（「取词兼容模式」，关闭后会失去 Safari 与 VS Code 支持）。仍然没有选中内容时，默认改读**剪贴板**里的纯文本，HUD 标为「朗读剪贴板」；密码框里不会这样做，密码管理器标记为隐藏的内容也不会被读 |
| **朗读字幕** | 默认开启。HUD 切换成只显示文字的大字幕，逐句显示正在读的那一句，普通朗读和翻译朗读都适用。系统默认语音（`say`）不回报进度，保留原来的紧凑 HUD |
| **翻译朗读** | 使用哪个大模型可单独选（跟随 LLM 页、指定提供商或自定义端点）；可按目标语言指定译文音色；单次最多 5000 字。模型失败、超时或未配置时在 HUD 上报错，**不会退回去朗读原文**。原文和译文默认保存到历史页（不保存音频，不参与同步），也可以选择把译文复制到剪贴板 |
| **朗读前整理**（可选） | 普通朗读前先让大模型去掉 Markdown、HTML 标记和链接地址，只读正文；整理失败时照读原文。朗读和翻译的提示词会把网页上复制来的表格读成完整的句子，而不是逐格念 |
| **朗读引擎** | 系统语音（离线、免配置，默认使用「系统设置 → 辅助功能 → 朗读内容」里的声音，在当前 macOS 上通常是 Siri 音色）；云端可选火山引擎豆包 Seed-TTS 2.0、阿里云百炼（默认 `qwen-audio-3.1-tts-flash`，单次可读更长文本、价格约为 3.0 的六分之一；`qwen-audio-3.0-tts-flash` 多 500 余个基础音色；`qwen3-tts-flash` 含北京、上海、四川、粤语等方言；`cosyvoice-v3-flash` / `cosyvoice-v3.5-flash`，后者用 Voice Design / 声音复刻的 `voice_id`）、小米 MiMo，或 Microsoft Azure Speech。云端引擎都是流式合成，按下热键后约 0.4–0.6 秒开始出声；超长文本按句切开连续合成，不再截断。每个引擎的音色、语速、音量独立设置；支持的阿里云模型还可以用一句自然语言描述朗读风格，默认是适合技术文本的平稳语调 |
...
- 选择云端引擎后，**要读的文字会发送给该供应商**用于合成。设置页在选中云端引擎时会明确写出这一点。
- 使用翻译朗读或朗读前整理时，文字还会发给所选的大模型提供商。
- 普通朗读**不保存**文本，也不保存合成音频；翻译朗读默认把原文和译文保存在本机历史（可关闭），不会上传到同步服务器。概览页只记录次数、字数和时长。普通日志只记录长度、取词来源、目标应用和错误码，不记录文本内容。
...
- **推理强度默认最低** — 纠错和翻译短文本用不上「思考」，只会多花几秒。每个提供商默认「自动最低」，按提供商、域名和模型换算成对方认可的写法（如火山方舟的 `thinking: disabled`），设置页会显示实际发送的字段；自定义端点还可以填写「额外请求字段」JSON 合并进请求体。不再给输出设固定上限，输出被截断时直接报错而不是拿半截结果去用。
...
- 概览仪表盘：总时长、字符数、AI 纠正次数、平均听写速度；朗读单列一组，统计普通朗读、翻译朗读次数、朗读字数、朗读时长和朗读触发的 AI 调用。均同时给出账户总计和本机数据。
- 翻译朗读的原文和译文也会出现在历史页，但只保存在本机。
...
- 朗读统计按设备上传到同步服务器汇总成账户总数；旧版服务器不认识这个接口时只是不汇总，历史同步不受影响。
