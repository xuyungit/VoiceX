...
每个里程碑单独成 PR。M1 与 M2 可以合并为一个 PR，如果改动量可控。M1、M2 的验证记录见附录 B，M3 的实现与验证见附录 B.3，M4 第一轮见附录 B.4，字幕开放给直接朗读与停止日志的收尾见附录 B.5，朗读结束时的空框修复见附录 B.6。
...
- 字数上限：M1 取 3000 字符，2026-09-20 放宽到 5000（附录 B.7）。超限在 HUD 提示"选中文字过长"，不发请求。上限是代码常量，不做设置项。M5 流式后取消。
...
- HUD 驱动线程按现有 60 ms 轮询读取后端当前段，变化时发 `state:caption` 事件，载荷为当前句文本、下标、总数；日志 `event=caption index=… total=…`。只有新的一句会替换字幕：朗读结束或被停止后，最后一句留在 HUD 上度过 400 ms 的停留，窗口隐藏之后才清空；一句都没显示过就结束的朗读（出声前被停止）不停留，直接隐藏；出错时字幕让位给错误提示（附录 B.6）。

版式：

- M3（已实现）：新增 `caption` 呈现，窗口 400×108，正文 12px、行高 1.45、最多三行，显示当前句；翻译阶段文字区显示"正在翻译..."，译文返回到第一段出声之间显示"正在准备朗读..."。已知限制：超过三行的段只显示尾部、开头是省略号（`fitText` 沿用保尾规则），中文约 90 字以上会截掉开头；分段按句不按段落，一段可能跨过段落换行。前者由 M4 第一轮的四行大字版式解决，后者保留。
- M4 第一轮（已实现）：字幕呈现只显示文字。状态行（图标、倒计时、意图标签）、波形、边框全部不显示；窗口 680×140，正文 20px、行高 1.4、居中、最多四行，中文 120 字的一段（字幕分段上限）放得下，更长的段仍按保尾规则只显示尾部。占位文字（"正在翻译..."、"正在准备朗读..."）同样放大、颜色稍暗；出错时文字变红，没有图标。窗口尺寸的上限钳制随字幕尺寸走（`window.rs`），避免加大后被静默缩回。位置与透明度沿用现状：底部居中、`hudTransparent` 控制整窗透明度。验证见附录 B.4。
- M4 后续：位置与透明度可调，与现有 `hudTransparent` 设置协调。这一步以视觉效果为准，允许多轮迭代。

风险：句级分块让云端请求数增加。CosyVoice 已经以 120 字分块运行，衔接没有问题；其他后端在实现时验证请求频率是否触发限流。M3 实测见附录 B.3：请求按段顺序发出，不受播放节奏约束，2870 字 35 段在阿里云上没有出现重试（`speak_retry`）。
...
- 目标语言初始列表见 3.2，默认英语
- 翻译字数上限 3000 字符（2026-09-20 起 5000，附录 B.7）
- 字幕段长上限 120 字符
...
- 开发版应用实测（阿里云，Cerebras 翻译，目标语言 zh-CN）：
  ...
  - 2870 字分节长文：翻译 1.25–2.3 秒；`speak_chunked pieces=35`；`speak_start` 到 `speak_started`（第一段请求、解码、出声）1 秒；字幕每段 10–14 秒推进，index 0→3 分别在出声后 0、14、24、37 秒；45 秒时按热键停止，`speak_stop` → `speak_stopped` → `speak_cancelled`，HUD 当秒隐藏。四次长文运行都没有重试事件（事件名是 `speak_retry`；本条最初误写为 `cloud_retry`，2026-09-18 更正，结论来自通读当时的事件行而不是按名字搜索）。
...
- 改动：`caption` 呈现改为 680×140；`body.caption-mode` 下状态行（图标、倒计时、意图标签）、波形、边框都不显示，正文 20px、行高 1.4、居中、最多四行，占位文字 50% 白。`hud.ts` 在字幕类切换的当下刷新测量元素，而不是等下一次窗口 resize，否则第一句按 12px 量、按 20px 画，会被裁掉而不是省略。`window.rs` 的尺寸上限改为跟随字幕尺寸，并加单测保证三种呈现都不会被钳制缩小。
...
### B.6 朗读结束时的空框（2026-09-18）

- 现象：字幕模式下朗读结束后，HUD 先变成没有文字的空框，约 400 ms 后才消失。
- 原因是三处各自合理的决定叠在一起：驱动线程在朗读结束时先清空字幕（M3），再按 `HUD_LINGER_MS` 停留 400 ms 才隐藏（8 月就有的防闪烁逻辑，早于字幕），而 M4 的字幕版式只画文字，清空之后窗口里什么都不剩。紧凑版停留时还有图标和波形，所以一直没有暴露。
- 改动（`controller.rs`）：抽出 `next_caption`，轮询里只有新的一句才替换字幕，后端不再报进度（音频结束比会话结束早一拍，原来这一拍也会把字幕清掉）不算变化；朗读结束时不再清空，改在停留结束的回调里先隐藏窗口、再清空。抽出 `lingers_after_read`：字幕模式下一句都没显示过（出声前被停止）就不停留，直接隐藏，否则同样是 400 ms 的空框。出错时先清字幕再显示错误；听写接管窗口时清字幕后返回。`hud.ts` 只改了两处注释，渲染逻辑不变：停留期间 `captionText` 还在，字幕版式照常画出。
- 新增两条单测：`next_caption` 对同一句、下一句、无进度三种输入的结果；`lingers_after_read` 对紧凑版、有字幕、无字幕三种情况的结果。`cargo test --lib` 362 通过、9 个 ignored；`pnpm build` 通过；`scripts/tts/translate_read.sh --case all` 五个用例全部通过。
- 直接朗读实测（阿里云，字幕开）：36 字读完，`caption index=0 total=1` → `speak_finished`，HUD 随后隐藏；262 字读到 8 秒热键停止，`speak_stop` → `speak_stopped` → `speak_cancelled`，HUD 一秒内隐藏；热键后约半秒、出声之前停止，没有 `speak_started` 与 `caption`，同样三条停止日志，HUD 当秒隐藏。停留期间画面上是否还是最后一句，脚本看不到，由维护者目测确认。

### B.7 字数上限放宽到 5000（2026-09-20）

- 3000 的来由：翻译是一次非流式调用，整段译完才出声，超时按字数放大、1920 字以上封顶 60 秒。M1 定值当天，推理模型把应用硬编码的 4096 输出上限花在思考上，2900 字要么被截断、要么约 44 秒（附录 B），3000 已经贴着能用的边。
- 之后两处改动把这个约束拿掉了：固定输出上限全部删除（B.1）；各家推理强度默认压到厂商下限（`e0b8a55`）。同样 2900 字现在 Cerebras 1.4 秒、Deepseek 6.2 秒、`mimo-v2.5-pro` 11 秒。
- 按字数线性外推，5000 字最慢的一家约 19 秒，离 60 秒封顶还有三倍余量，所以 `TRANSLATE_MAX_CHARS` 改为 5000。这是外推，没有对 5000 字做实机测量；`VOICEX_LIVE_TEXT` 加 `llm_stage::tests::live` 可以按需补测。TTS 一侧不受影响：朗读本来就分段请求，直接朗读从来没有字数上限。
- 同步改动：设置页说明里的数字（`ReadingSettings.vue`）、HUD 超限文案（两种语言）、`translate_read.sh` 的 toolong 选区从 3200 字加到 5200 字、`tts-regression-testing.md` 的用例表。long 用例仍是约 2850 字，它抓的是输出截断，与上限无关。
- 再往上放的代价：5000 字要读十七八分钟，超时封顶仍是 60 秒，开着思考的慢模型（B.1 里 mimo 开思考 600 字 17 秒）在这个长度必然超时。真正去掉上限还是 M5 的流式。
